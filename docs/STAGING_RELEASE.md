...
- Vercel installs with `npm ci`, not `npm install`; prebuilt release paths refuse to upload if building changed the checkout. Windows operators upload clean, CI-approved source for a Linux remote build, avoiding cross-platform native bindings and the Windows function builder's secondary lockfile rewrite. Linux operators and CI use prebuilt artifacts.
